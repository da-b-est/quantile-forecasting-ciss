...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 30 07:17:07 2024
...
df['current_gRGDP_yoy'] = df['g_RGDPyoy']


# Define which column is the dependent variable (based on h)
h = 4  # GDP Growth 4 Quarters Ahead 
...
plt.title('Panel D. One Year ahead: CISS')
# Add a legend
...
# Formatting
plt.title('Panel B. One Year Ahead')
plt.xlabel('Time')
plt.ylabel(label_text)
plt.ylim([-15, 20])  # Adjust y-axis bounds as needed
plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter())  # Format y-axis as percentage
...
df.dropna(subset=['current_gRGDP_yoy', ycol], inplace=True)

#%% Replicating 'Figure 2: Raw Data' in Adrian et al. (2019)
#Plot the GDP growth and the current_gRGDP_yoy
# Function to add recession shading to plots
def add_recession_shading(ax, recession_dates, color='gray', alpha=0.3):
    """
    Add shaded areas to represent recession periods on a given axis.
...
# Add recession shading
add_recession_shading(ax, recession_dates)
# Plotting current_gRGDP_yoy on the secondary y-axis
ax2 = ax.twinx()
df['current_gRGDP_yoy'].plot(ax=ax2, label='current quarter GDP growth', linewidth=1, color='r', linestyle='--')
...
#%% Scatter plot setup
yaxis_bounds_aux = [-20, 20]  # Define y-axis bounds
plt.scatter(df.current_gRGDP_yoy, df[ycol], c='b', label='Data 1973-2024')
# Labels and title
...
# Plot current_gRGDP_yoy and shifted GDP growth on two different y-axes
df.loc[:, ['current_gRGDP_yoy', ycol]].plot(secondary_y='current_gRGDP_yoy')
plt.title('current_gRGDP_yoy'' vs GDP Growth')
plt.show()


#%% Linear regression
# Prepare the matrix (current_gRGDP_yoy column for X, and ycol for y)
X = df['current_gRGDP_yoy']
y = df[ycol]
...
# Scatter plot of current_gRGDP_yoy vs GDP Growth
plt.scatter(X, y, c='b', label='Data 1973-2024')
# Plot the regression line
plt.plot(X, predictions, 'r', label='Mean E[Y|X]', linewidth=2)
# Labels and title
...
X = df['current_gRGDP_yoy']  # Replace with the correct data selection logic
...
plt.ylim([-10, 10])  # Adjust y-axis bounds if needed
plt.title('Panel B. One year ahead: GDP')
plt.legend(loc='lower left', fontsize=8)
